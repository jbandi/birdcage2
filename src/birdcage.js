(function (global) {
    ...
    var config = {
        apiKey: "AIzaSyDqq9FawE1g95hIK9UyBbqPYjCFFEOzzqM",
        authDomain: "scorching-inferno-3523.firebaseapp.com",
        databaseURL: "https://scorching-inferno-3523.firebaseio.com",
        storageBucket: "scorching-inferno-3523.appspot.com"
    };
    ...
    function updateUser(user) {
        _firebase.database().ref('users/' + _authData.uid).update(user);
    }
    ...
    function postTweet(text) {
        ...
        _firebase.database().ref('posts/' + _authData.uid).push(tweet);
    }
    ...
    function increasePriority(tweet) {
        ...
        if (tweet['.priority'] <= 1) {
            ...
            _firebase.database().ref('posts/' + _authData.uid + '/' + tweet.id).update({'.priority': highestPriority + 1});
        }
        else {
            var index = _tweets.indexOf(tweet);
            if (index > 0) {
                var prevPrio = _tweets[index - 1]['.priority'];
                _firebase.database().ref('posts/' + _authData.uid + '/' + tweet.id).update({'.priority': prevPrio - 1});
            }
        }
    }
    ...
    function deleteTweetById(tweetId) {
        _firebase.database().ref('posts/' + _authData.uid + '/' + tweetId).remove();
    }
    ...
    function randomize() {
        $.each(_tweets, function (index, tweet) {
            ...
            _firebase.database().ref('posts/' + _authData.uid + '/' + tweet.id).update({'.priority': randomPrio});
        })
    }
    ...
    function loadTweets(all) {
        ...
        _firebaseListener = _firebase.database().ref("posts/" + _authData.uid)
            .orderByPriority()
            .startAt(all ? 0 : 2);
